...
The above is the same graph above but for all of MAG. The next step is to 
integrate this into the existing graph. Options:

- Normalise existing graph by baseline
- Add baseline as a background (maybe with different line type)

The latter seems more in line with the focus of the analysis, since the target
is the overall distribution of knowledge production, not the relative 
specialisation.
